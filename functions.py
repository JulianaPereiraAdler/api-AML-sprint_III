...
def diligencia_compliance_inicial(cpf, nome_completo, API_Key):
    ...
    url = "https://amlduediligence.com.br/drive/get_mm.php?usuario="+decrypt(API_Key, "0IMPOIfsdf3kopfFDF1MIF#If")+"&pessoa="+nome_completo+"&cpf_cnpj="+cpf
    ...
    if int(qtd_value) == 0: 
        html_resposta_aml = html_resposta_aml + "<p style='color:#326b1f; padding-bottom:10px;'><b>"+nome_completo.upper() +" não consta em Mídias Tratadas e Listas de Sanções Nacionais.</b>"
    ...
    if int(qtd_value) > 0:   
        for cadastro in root.findall('cadastros/cadastro'):
            ...
            html_resposta_aml = html_resposta_aml + "<p style='color:#842525; padding-bottom:10px;'><b>Foram encontradas " + str(qtd_value) + " pessoas para a busca por ''" + nome_completo + "'.<p></b>" 
            html_resposta_aml = html_resposta_aml + f"<p>Nome: {nome}</p>"
    ...
    if int(qtd_value) == 0: 
        html_resposta_aml = html_resposta_aml + "<p style='color:#326b1f'>"+nome_completo.upper() +" não consta como PEP."
    ...
    if int(qtd_value) > 0:   
        for cadastro in root.findall('cadastros/cadastro'):
            ...
            html_resposta_aml = html_resposta_aml + "<p style='color:#842525; padding-bottom:10px;'><b>Foram encontradas " + str(qtd_value) + " pessoas para a busca por ''" + full_name + "'.</b><p>" 
            html_resposta_aml = html_resposta_aml + f"<p>Nome: {nome}</p>"
